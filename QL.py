"""
STATIC / BASELINE TRAFFIC SIGNAL CONTROLLER (DQN-STYLE METRICS)

- Uses a DQN model structure but you can treat this as your "static/baseline" RL controller.
- Faster simulation:
    STEP_LENGTH = 1.0 s
    TOTAL_STEPS = 3000
    MIN_GREEN_STEPS = 5 (5 seconds)
- Outputs (same style as Q-LEARNING METRICS):
...
STEP_LENGTH = 1.0       # seconds per simulation step (faster sim)
TOTAL_STEPS = 3000      # reduce for quick runs

# If SUMO_HOME is not set, you can uncomment and set it here:
...
# -------------------------
# Import TraCI and find SUMO binary
# -------------------------
if os.name == 'nt':
...
print("\n===== SUMO CONFIGURATION (STATIC / BASELINE) =====")
...
# -------------------------
# RL Hyperparameters (DQN-style controller)
# -------------------------
ALPHA = 0.1
GAMMA = 0.9
EPSILON = 0.1
ACTIONS = [0, 1]      # 0 = keep phase, 1 = switch phase
MIN_GREEN_STEPS = 5   # 5 seconds minimum green
...
def get_state():
    ...
    """
    q_EB_0 = safe_get_detector(DETECTOR_IDS["Node1_2_EB_0"])
    q_EB_1 = safe_get_detector(DETECTOR_IDS["Node1_2_EB_1"])
    q_EB_2 = safe_get_detector(DETECTOR_IDS["Node1_2_EB_2"])
    q_SB_0 = safe_get_detector(DETECTOR_IDS["Node2_7_SB_0"])
    q_SB_1 = safe_get_detector(DETECTOR_IDS["Node2_7_SB_1"])
    q_SB_2 = safe_get_detector(DETECTOR_IDS["Node2_7_SB_2"])
    current_phase = safe_get_phase(TRAFFIC_LIGHT_ID)
    return (q_EB_0, q_EB_1, q_EB_2, q_SB_0, q_SB_1, q_SB_2, current_phase)

def get_reward(state):
    total_queue = sum(state[:-1])  # exclude phase
...
def apply_action(action, tls_id=TRAFFIC_LIGHT_ID, current_step=0):
    """
...
def compute_metrics(queue_history, travel_times, step_length=STEP_LENGTH):
    """
    Same style as Q-LEARNING METRICS:
      - Total delay (veh-seconds)
      - Average queue length (vehicles)
      - Max queue length (vehicles)
      - Vehicles completed (throughput)
...
travel_times = []   # list of finished vehicle travel times

print("\n🚦 Starting SUMO (STATIC / BASELINE DQN-STYLE)...\n")

try:
    traci.start(Sumo_config)
    print("✅ SUMO started successfully.")

    # Make GUI faster if available
    try:
        traci.gui.setSchema("View #0", "real world")
        traci.gui.setSpeed("View #0", 5)  # 5x faster visualization
    except Exception:
        pass
    ...
    for step in range(TOTAL_STEPS):
        ...
        apply_action(action, tls_id=TRAFFIC_LIGHT_ID, current_step=step)
        ...
        if step % 200 == 0:
            ...
            print(
                f"Step {step}, Time={current_time:.1f}s, State={state}, "
                f"Action={action}, Reward={reward:.2f}, CumReward={cumulative_reward:.2f}, "
                f"Q={Q_vals}, Finished={len(travel_times)}"
            )

        # Early stop if no more vehicles expected
        try:
...
# -------------------------
print("\nQ-Learning Training completed. Final Q-table size:")
dqn_model.summary()
...
if len(time_history) > 0:
    ...
    # Save for comparison (optional)
    np.savez("results_static.npz",
    ...
    plt.plot(time_history, reward_history, label="Cumulative Reward", marker='o')
    plt.xlabel("Time (s)")
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(time_history, queue_history, label="Total Queue Length", marker='o')
    plt.xlabel("Time (s)")
    ...
    print("No steps recorded; nothing to analyze.")
